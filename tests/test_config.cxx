...
block {
  other-int 1024;
...

    {
        ...
        REQUIRE(s.size() == 0);
        REQUIRE(s.name() == "qstring-value");
        REQUIRE(s.value() == "this is a qstring");
    }

    {
        auto const &s = c->items[3];
        REQUIRE(s.size() == 0);
        REQUIRE(s.name() == "bool-value");
...

    {
        ...
        REQUIRE(block.value() == "test");

        REQUIRE(block.size() == 1);

        auto const &i = block[0];
        REQUIRE(i.name() == "sub-int");
